...
1: cancer_sample 2: flow_sample 3: building_permits 4: parking_meters
...
In our exploration, we would intentionally separate the ID column out
since when working with the data manipulation and visualization, ID is
something we cannot and should not include normally.
...
``` r
### EXPLORE cancer_sample HERE ###
analysis <- cancer_sample %>% select(diagnosis,ends_with("_mean"))
head(analysis)
```
